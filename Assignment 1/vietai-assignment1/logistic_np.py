...
class LogisticClassifier(object):
    ...
    def compute_loss(self, y, y_hat):
        ...
        z=np.reshape(y,(1,y.shape[0]))
        losss= np.float64
        loss = +(np.dot(z,np.log(y_hat))+np.dot((1-z),np.log(1-y_hat)))
        loss = np.float64((np.reshape(loss,1))/(-z.shape[1]))
        return loss
    ...
    def get_grad(self, x, y, y_hat):
        ...
        grad=np.dot(np.transpose(x),(y_hat-y))/(y.shape[0])
        return grad
...
def reshape2D(tensor):
    ...
    m=tensor.shape[0] # number of training pic
    R=tensor.shape[1] # number of pixel in 1 pic in horizontal
    C=tensor.shape[2] # number of pixel in 1 pic in horizontal
...
def add_one(x):
    ...
    # [TODO 1.4]
    x=np.array(x)
    y=np.array(np.ones((list(x.shape)[0],1)))
...
def test(y_hat, test_y):
    ...
    recall = 0
    f1 = 0
    m,TP,FP,TN,FN = 0,0,0,0,0
    m = y_hat.shape[0]
    threshold = 0.5
    for i in range (m):
        if y_hat[i]>=threshold: y_hat[i]=1
        else: y_hat[i]=0
        if y_hat[i] == 1:
            if y_hat[i]==test_y[i]:
                TP+=1
