...
class SoftmaxClassifier(LogisticClassifier):
    ...
    def feed_forward(self, x):
        ...
        z=np.dot(x,self.w) #z.shape = m*K
        zmax=np.amax(z,axis=1).reshape(x.shape[0],1) #zmax.shape = m*1
        zmax=np.tile(zmax,self.w.shape[1]) #zmax.shape = m*K
        z_hyphen=np.exp(z-zmax) #z_hyphen.shape = m*K
        s=np.sum(z_hyphen,axis=1).reshape(x.shape[0],1) #s.shape = m*1
        sm=np.tile(s,self.w.shape[1]) #sm.shape = m*K
        y_hat=np.divide(z_hyphen,sm) #y_hat.shape = m*K
    ...
    def compute_loss(self, y, y_hat):
        ...
        sum1= np.sum(np.multiply(y,np.log(y_hat)),axis=1).reshape(y.shape[0],1)
        loss=np.float64(-np.mean(sum1,axis=0))
        return loss
    ...
    def get_grad(self, x, y, y_hat):
        ...
        y_hyphen = np.subtract(y_hat,y)
        z=np.dot(x_transpose,y_hyphen)
        grad = z/x.shape[0]
        return grad
...
def test(y_hat, test_y):
    ...
    for i in range (m):
        ...
        l=np.argmax(y_hat[i])
        k=np.argmax(test_y[i])

        confusion_mat[k][l]+=y_hat[i][l]
    max_row = np.sum(confusion_mat,axis=1)
    for i in range (10):
        for j in range (10):
            confusion_mat[i][j]=np.true_divide(confusion_mat[i][j], max_row[i])
...
if __name__ == "__main__":
    ...
    for e in range(num_epoch):    
        ...
        if val_loss > (1.3*train_loss): break
